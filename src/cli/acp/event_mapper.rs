//! `ServerEvent` -> ACP `session/update` translation.
//!
//! Split out of `acp.rs` to keep that file's JSON-RPC transport concerns
//! separate from the event vocabulary, and because the mapper is the part most
//! often extended when a new `ServerEvent` variant appears.

use super::{AcpProfile, agent_message_chunk, parse_json_object, tool_kind, tool_title};
...
impl EventMapper {
    pub(super) fn new(profile: AcpProfile) -> Self {
        Self {
            profile,
