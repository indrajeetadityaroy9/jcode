...
use crate::{memory, session, storage, tui};
...
/// Run one ambient cycle in a visible TUI. Invoked only by the ambient runner,
/// which spawns `jcode ambient run-visible` in its own terminal window.
pub async fn run_ambient_visible() -> Result<()> {
