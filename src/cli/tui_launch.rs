...
use crate::{id, logging, replay, server, session, setup_hints, startup_profile, tui};
...
pub async fn run_tui_client(
    ...
    if let Some(ref session_id) = resume_session {
        ...
        {
            crate::process_title::set_client_remote_display_title(&server_info.name, &session_name);
        } else {
            crate::process_title::set_client_display_title(&session_name);
        }
