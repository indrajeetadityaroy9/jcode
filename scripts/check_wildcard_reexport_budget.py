...
MUST_COUNT = (
    "pub use jcode_base::*;",
    "pub use jcode_provider_core::protocol::*;",
    "    pub use ::jcode_tui::style::*;",
)
MUST_IGNORE = (
    "use jcode_base::*;",              # not a re-export
    "pub use crate::session::*;",      # not a sibling crate
    "pub use jcode_base::Session;",    # not a glob
)
...
def main() -> int:
    args = parse_args()
    bc.self_check("wildcard re-export", lambda line: bool(PATTERN.search(line)), MUST_COUNT, MUST_IGNORE)
    ...
    if baseline is None:
        ...
        return 1
    if not args.repair:
        bc.validate_baseline(baseline, BASELINE_FILE, table_key="files")
    if bc.run_ledger_edits(
        args,
        baseline,
        counts,
        lambda data: bc.write_json(BASELINE_FILE, data),
        table_key="files",
    ):
        return 0
