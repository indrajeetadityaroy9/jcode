...
# The three onboarding-flavored alternatives that used to live in
# EXCLUDED_DESC are gone: onboarding is now PURGED_DESC, and `mark()` tests
# PURGED before EXCLUDED, so they could never match again.
#
...
PURGED_DESC='telemetry|sponsor|gateway|discovery|integration tools|request_permission|ambient permission|iOS|pair your phone|powershell|install\.ps1|windows hotkey|windows launcher|authenticode|smartscreen|self-dev|selfdev|canary session|launch hotkey|global hotkey|setup-hotkey|launch shortcut|grok|xai|x\.ai|desktop app|desktop2|vello|parley|aws bedrock|bedrock provider|bedrock api key|--provider bedrock|browser automation|browser tool|firefox bridge|agent bridge|chatgpt web|gmail|composio|google/gmail|email notification|smtp|imap|mail transport|computer use|desktop control|screen recording|accessibility permission|bundled docs|jcode_docs|first-run|first run|onboarding|welcome screen|setup wizard'
EXCLUDED_DESC='defaults to the [Jj]code subscription'
...
PURGED_IDENT='telemetry|sponsors|gateway|discover|integration_tools|request_permission|jcode-tui-permissions|phone-server|scripts/remote/|jcode-pair|windows_setup|windows_hotkeys|\.ps1|listen_windows_hotkey|selfdev|SelfDev|set_canary|is_canary|launch_hotkeys|LaunchHotkey|setup-hotkey|global-hotkey|grok-build|GrokBuild|XAI_|jcode-provider-grok-build-runtime|jcode-desktop2|jcode-math|jcode-provider-bedrock|BedrockProvider|BEDROCK|Desktop2|BrowserTool|chatgpt_web|CHATGPT_WEB|ensure_browser_ready|browser_binary_path|firefox_agent_bridge|GmailTool|GmailBackend|GmailClient|GmailAccessTier|GOOGLE_LOGIN_PROVIDER|COMPOSIO_|JCODE_GMAIL_BACKEND|jcode-notify-email|jcode_notify_email|poll_imap_once|imap_reply_loop|lettre|macos_computer_use|ComputerTool|core-graphics|jcode_docs|JcodeDocsTool|^[[:space:]]*(Pair|Permissions|Telemetry|Bedrock|Browser)\b|Onboarding[A-Z]|onboarding_|ui_onboarding|is_new_user_install|prefer_strongest|repo_ranking'
