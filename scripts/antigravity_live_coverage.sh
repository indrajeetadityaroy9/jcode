...
for m in "${MODELS[@]}"; do
  ...
  # --- chat smoke ---
  chat_out=$(timeout "$CHAT_TIMEOUT" "$JC" run --provider antigravity -m "$m" --no-update "$CHAT_PROMPT" 2>&1)
  chat_rc=$?
  ...
  note=""
  tool_out=$(timeout "$TOOL_TIMEOUT" "$JC" run --provider antigravity -m "$m" --no-update "$TOOL_PROMPT" 2>&1)
  tool_rc=$?
