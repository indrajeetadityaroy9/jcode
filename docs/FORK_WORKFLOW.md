...
| **Non-core flags: updater, Cloud/Jade, launcher, ambient verbs** | ~8,900 lines, chosen by a flag-surface audit that asked of each candidate "can this execute on this machine at all?" rather than "is it useful?". (1) **Proven-dead leaves.** The `--debug-socket` global flag (`args.rs:70`), declared and read *nowhere* — the real gate is the `display.debug_socket` config key plus `JCODE_DEBUG_CONTROL` and `~/.jcode/debug_control` in `server/util.rs:13-31`, none of which ever consulted the flag. Four dead env vars: `JCODE_AMBIENT_PROACTIVE` and `JCODE_AMBIENT_PROVIDER` (single hit each, only the `CONFIG_ENV_KEYS` fingerprint array), `JCODE_OPENROUTER_DEFAULT_MODEL` (two hits, both test env save/restore arrays), `JCODE_MMDR_SIZE_API_AVAILABLE` (a `rerun-if-env-changed` for a name nothing reads — `jcode-tui-mermaid/build.rs:9` declared it, `:10` read a different one). Plus a duplicate `JCODE_AUTO_POKE` row in `CONFIG_ENV_KEYS`. (2) **Cloud/Jade, ~2,800 lines**: `server/jade_relay.rs` (1,396), the `JadeRelayChannel` half of `channel.rs` (441 of its 868 lines — Telegram and Discord are untouched and their registration match is unchanged), the ~1,369-line Cloud/Jade CLI span in `src/cli/commands.rs` including an HTML dashboard renderer, `Command::Cloud`/`CloudCommand`/`CloudSessionsCommand`/`JadeCloudOptions`/`CloudSessionViewFormat`, the nine `SafetyConfig.jade_relay_*` fields and the nine `JCODE_JADE_RELAY_*` overrides. Proof it could not run: `resolve_jade_sessions_helper` requires `~/jade/scripts/jade_sessions.py`, which exists neither on this machine nor in the repo, and `spawn_if_configured` needs a relay api_base + token that are unset. (3) **The self-updater, ~2,650 lines**: `jcode-app-core/src/{update.rs (1,724), update_metadata.rs, update_rate_limit.rs}`, the whole `jcode-update-core` crate (632) with its workspace member and dependency, `spawn_background_update_check`, the update half of `hot_exec.rs` (381 lines — the file went 505 → 124), `Command::Update`, `--auto-update`, `features.{check_updates,update_channel}` + `UpdateChannel`, `BusEvent::UpdateStatus`, `ClientMaintenanceAction::Update`, and the TUI `/update` command with the Ctrl+Y "update diverged, let an agent merge" offer. Unreachable three ways over: `jcode version --json` reports `release_build: False` and `should_auto_update` returns false on a non-release build before any network call; `api.github.com/repos/indrajeetadityaroy9/jcode/releases/latest` returns **404** because this fork publishes no releases; and the owner's config had `check_updates = false`. (4) **`setup-launcher`** — a pure duplicate: `maybe_show_setup_hints` already calls `create_desktop_shortcut` on every startup, so the manual command and its dead invocations in `scripts/install{,_release}.sh` went while the automatic path stayed. (5) **The four user-facing `ambient` verbs** (`status`, `log`, `trigger`, `stop`) and `run_ambient_command`: `ambient.enabled = false` and all four routed through `debug_control_allowed()`, which is off. (6) **Redundant second entry points**, removed without any behavior change because a first entry point already reaches the same state: the 25 `JCODE_*_KEY` keybinding env overrides (`env_overrides.rs:10-85` plus their `CONFIG_ENV_KEYS` rows) — keybindings are set in `[keybindings]`, which is where the owner sets all 28 of theirs, and unlike `--tools` there is no per-invocation case for rebinding a chord; the two exact alias pairs in `login_qr.rs` (`JCODE_LOGIN_QR` → `JCODE_SHOW_LOGIN_QR`, `JCODE_TUI_LOGIN_QR` → `JCODE_SHOW_TUI_LOGIN_QR`); the explicitly-labelled legacy alias `JCODE_RUNTIME_MEMORY_LOG_INTERVAL_SECS`, whose `_PROCESS_` and `_ATTRIBUTION_` successors it was silently feeding; and `auth-test --coverage/--coverage-file/--coverage-limit` with the 34-line `run_auth_test_coverage_command` behind them, a byte-equivalent re-entry into the `provider-test-coverage` command, which keeps both flags and still runs. No guard section was added for this group on purpose: a pattern broad enough to catch `JCODE_*_KEY` also matches every `JCODE_*_API_KEY`, and a guard that cries wolf gets ignored. **What was deliberately kept, and why each would have been a mistake to cut:** `--no-update` — `hot_exec.rs:57` still emits it into its own re-exec argv, so the flag is an internal contract, not a user preference. Every `reload`/`rebuild` path — `jcode server reload` and `session_rebuild.rs` are this fork's *actual* build-from-source route, so `run_git_pull_ff_only`, `summarize_git_pull_failure`, the divergence helpers and `print_centered` were **relocated** into `session_rebuild.rs` verbatim rather than deleted, and their `git_pull_failure_summaries_are_stable` test moved with them. `ambient run-visible` and the whole ambient **runner** — `server.rs:743-747` builds it unconditionally and `startup_tests.rs:107-110` pins that with "schedule/session tasks need the runner even when ambient is disabled", because the model-callable `schedule`/`schedule_ambient`/`end_ambient_cycle`/`send_message` tools ride on it; `AmbientCommand` therefore survives with exactly one hidden variant, the runner's own re-exec target. The `ctrl+y` **`fallback_switch`** keybinding — the merge offer only borrowed that chord contextually, and provider fallback still owns it (`config-types/src/lib.rs:987`, and the owner binds it explicitly). The `update_channel_symlink` machinery in `jcode-build-support` — same word, unrelated concept: it manages the `stable`/`current`/`shared-server` install symlinks that `scripts/install_release.sh` depends on. **Config compatibility** is covered by a new regression test, `removed_feature_keys_are_ignored_instead_of_failing_config_parse` (issue #349): an existing `config.toml` still carrying `check_updates`/`update_channel` must parse, because a hard parse failure during the reload handoff previously left the reload marker stuck in `starting` and clients re-requested the reload forever. Two documentation files went with their subsystem: `docs/dev/ACCOUNT_CONTRACT_CONFORMANCE_TESTS.md` and `docs/dev/ACCOUNT_FLOWS_OBSERVABILITY_PRIVACY.md` (518 lines describing the already-deleted subscription account contract, and citing line ranges in files that no longer exist). |
| **Dictation and the replay video encoder** | ~2,600 lines. (1) **Dictation**, dead as configured (`[dictation] command = ""`, `key = "off"`, so `run_configured` bailed on entry): `jcode-base/src/dictation.rs` + tests, `jcode-tui/src/tui/app/dictation.rs` (508 lines: `ActiveDictation`, `DictationExit`, the spawn/read/wait/publish machinery), `Command::Dictate` and `run_dictate_command`, the `/dictate` + `/dictation` slash commands and their help/overlay rows, the dictation keybinding (`keybind::load_dictation_key`, the `TuiState::dictation_key_label` trait method and both impls, the `hotkey_feedback` entry), five `App` fields, `BusEvent::Dictation{Completed,Failed}` and both bus arms, the whole `[dictation]` config surface (`DictationConfig`, the four `JCODE_DICTATION_*` overrides, the generated template block, the `display_summary` rows) and — chain-followed — `RemoteConnection::send_transcript`, whose only caller was the deleted remote bus arm. (2) **The video encoder**: `jcode-tui/src/video_export.rs` (1,161 lines, the only code in the repo that shelled out to `rsvg-convert` and `ffmpeg`), `App::run_headless_replay`, `replay::compose_swarm_buffers` + `SwarmPaneFrames`, the `--video`/`--cols`/`--rows`/`--fps` flags, and three demo scripts (`record_demo.sh`, `replay_recording.sh`, `capture_demo.sh`, 385 lines) that were **already dead** for a different reason: they drive `niri msg`, the Wayland compositor removed in the platform purge, and use GNU-only `grep -oP`. **The fences the owner set, and how each was honored:** *image handling stays* — the `image` crate (png+jpeg) and all eight `image::` call sites in `ui_inline_image.rs`, `ui_pinned.rs`, `app/helpers.rs` and `app/productivity.rs` are untouched, and **zero dependency lines changed in any manifest** (the encoder owned no crate dep; it shelled out to binaries). *`replay` playback stays* — `--export` prints timeline JSON and `--speed`/`--swarm`/`--timeline`/`--auto-edit`/`--centered` still drive interactive playback. *Text injection stays* — `jcode transcript`, `Request::Transcript` and its live CLI producer `client_api::send_transcript` all survive, so any external STT script can still drive the TUI; dictation's `remember_last_focused_session`/`last_focused_session` were **relocated into `jcode-base/src/storage.rs`** rather than deleted, because they route that injection. Two consequences worth knowing: `jcode-tui-mermaid`'s `VIDEO_EXPORT_MODE` was **renamed** to `IMAGE_PLACEHOLDER_MODE` (with `write_video_export_marker` deleted, since its `JMERMAID:` marker had exactly one reader — the deleted encoder) because after the purge it is an image-placeholder rendering mode, not a video component — note its only writer is a test helper, so the production branches are always-false today; and the harness-API capability ledger lost its `("Transcript", ClientInternal)` row, caught by `every_reference_client_capability_is_triaged`, because the TUI no longer sends that request even though the CLI still does. The `/record` debug command survives but now writes a raw harness event dump with no consumer — its message says so plainly instead of pointing at the deleted script. (3) **The demo recorder and the media it produced**, removed in a second pass once it was clear nothing could consume its output: the `/record` slash command with its `start`/`stop`/`cancel` arms, the `record-start`/`record-stop`/`record-events` debug-socket commands, `test_harness`'s `EventRecorder` + `EVENT_RECORDER` global + the four free functions, and the per-keystroke `record_event` hook in `handle_key_event` — every TUI keypress was being offered to a recorder whose only consumer was the deleted encoder. Chain: removing the capture side orphaned `lock_unpoisoned` and the `Mutex` import, both dropped; `RecordedEvent`, `TestEvent`, the test clock and `TestScript`/`TestStep` playback all **stay** — that is test infrastructure, not video, and `TestScript::from_json` now parses `Vec<RecordedEvent>` directly. And **167 MB of orphaned media**: 14 `.mp4`/`.gif` files under `assets/demos/` and `assets/readme/` plus four encoder-input `*_timeline.json` files, every one with zero references anywhere in the repo (the README embeds remote `user-attachments` URLs, not local files). `assets/` went 167 MB → 2.1 MB. **`assets/` is now gone entirely** (removed in a later pass). `demos/` (196 KB) had zero references from any Rust file, script, manifest, or doc. `app-icons/Jcode.icns` (1.9 MB) did: it was `include_bytes!`'d into `jcode-setup-hints/src/macos_launcher.rs` and written into the `Jcode.app` launcher and `Jcode Notifications.app` broker bundles, named as `CFBundleIconFile` in both `Info.plist`s, and required by `macos_app_launcher_is_valid`/`macos_notification_broker_is_valid`. Removing it therefore also took the const, both `Resources/` writes, both plist keys, and the icon term in both validity predicates — that last part matters, because leaving it would make each bundle look permanently invalid and reinstall on every launch. Both bundles now ship with the generic macOS icon; the binary is ~1.9 MB smaller. Note the blobs remain in git history, so this shrinks a fresh checkout, not the clone; reclaiming that needs a history rewrite the owner has not asked for. A `video media assets must stay deleted` guard section now fails on any tracked `.mp4/.mov/.webm/.gif/.avi/.mkv`. (4) **The last audio feature: the notification sound.** `NotificationsConfig.turn_complete_sound` (default `"Glass"`) and the whole chain it fed: the read in `turn_notify.rs`, the `sound` parameter of `send_originating_terminal_notification`, `send_macos_turn_notification` and `send_desktop_notification_rich`, the `MacosNotificationEnvelope.sound` field that carried it through the on-disk broker inbox, the AppleScript ` sound name "…"` fragment, and — chain-followed into `src/cli/macos_notification_broker.rs` — `content.setSound`, the `UNNotificationSound` import, `UNNotificationPresentationOptions::Sound` and `UNAuthorizationOptions::Sound`, so the broker no longer **requests** audio permission it cannot use. **Turn-complete notifications still fire on all three paths** with title/subtitle/body and click-to-focus routing intact: kitty OSC 99, iTerm OSC 9, and the macOS LSUIElement broker (`send_macos_turn_notification(title, subtitle, body)`, `send_desktop_notification_rich(title, subtitle, body)`). Nothing else in the repo could produce sound: kitty's OSC 99 `s=` key was never emitted, iTerm's OSC 9 has no sound field, the ntfy/Telegram/Discord channels carry only priority, and there is **no audio crate dependency anywhere** (no `cpal`, `rodio`, `symphonia`, `hound`, `coreaudio`, `whisper-rs`). (5) **Screenshot automation**, the same shape of dead producer as the demo recorder: `jcode-tui/src/tui/screenshot.rs` (133 lines) only ever wrote *signal files* into `runtime_dir()/jcode-screenshots`; the code that actually captured a window was `scripts/screenshot_watcher.sh` / `auto_screenshot.sh`, both of which drive `niri msg action screenshot-window` — the Wayland compositor deleted in the platform purge. So the TUI was dropping signals nobody on macOS could consume. Gone with it: the `/screenshot-mode` and `/screenshot <state>` commands and their registrations, and `scripts/verify_alt_shift_e_reaches_terminal.sh` (13 `niri` calls). These three scripts were platform residue the earlier waves missed because the guards only scanned `*.rs`; the guard now matches `niri msg` in shell too. Two fixtures were de-nirified rather than deleted: the `focus_hook` config test's `"niri-focus-jcode"` value is now `"focus-jcode.sh"` (any string works — the hook is user-supplied). **Image handling is unaffected**: verified by 329 passing tests across `jcode-tui-mermaid` (121), `jcode-tui-markdown` (59), the `jcode-tui` inline-image/pinned/placeholder/mermaid filters (138) and app-core notifications (11). |
| **Dead surface: orphan SDK, orphan scripts, residue** | ~16,600 lines, from a catalog that asked of each candidate "who consumes this?" and required a quoted answer. (1) **`crates/jcode-sdk`** (4,398 lines) — a workspace member with **zero dependents** and no Rust reference outside itself; its only consumer was the desktop app, deleted earlier. The `jcode api-bridge` subcommand and `run_bridge` stay, so the socket protocol is untouched; only the standalone `jcode-harness-api-bridge` binary target went, because its only spawner was `jcode-sdk/src/launch.rs:476`. (2) **43 orphaned scripts** (11,670 lines) referenced by no script, doc, manifest, or Rust file: 18 one-off investigations (`repro_*`, `profile_*`, `diagnose_*`, `benchmark_*`), 22 misc (`reload_recovery_audit.py`, `jcode_monitor.py`, `jcode_harbor_claude_agent.py`), 4 visual verifiers (`verify_donut_still_animates.py`). **Kept** the 13 human-invoked `test_*` harnesses — reference-counting is the wrong test for those — and `auth_regression_matrix.sh`, which is the sole consumer of the `auth-test --coverage` flags. (3) **`tests/e2e/windows_lifecycle.rs`** (327 lines) — behind `#[cfg(windows)]`, so unbuildable here; residue of the Windows purge. (4) **`src/bin/test_api.rs`** (37 lines) — smoked the *deprecated* legacy Claude CLI provider, which itself only instantiates under `JCODE_USE_CLAUDE_CLI=1`; `jcode-harness` survives but is now behind `dev-bins` so a plain `cargo build` stops linking a second full copy of the graph. (5) **Residue**: the `"jcode subscription"` match arm (`provider/mod.rs:2830`) and the doctor roster entry naming that purged provider. (6) A **vacuous test guard** (`agent_tests.rs:773`) asserting no tool named `debug_socket` is model-visible — no such tool has ever existed here, so it could not fail; replaced with the same assertion over the three genuinely hidden ambient-only tools. **Corrected mid-purge:** `jcode_plan::dag::sim` was catalogued as dead on the basis that nothing outside the crate consumes it, but `dag/tests.rs` drives it from 34 call sites including `sim::run`; deleting it would have destroyed the DAG suite's harness. Restored and made `#[cfg(test)]` instead — zero release weight, 78 tests intact. |
| **Copilot, Cursor, and Azure OpenAI providers** | All three third-party model providers, with their auth, config, usage, and diagnostic surfaces. Crates deleted: `jcode-provider-copilot`, `jcode-provider-copilot-runtime`, `jcode-provider-cursor-runtime`, `jcode-azure-auth` (workspace 74 → 70 members; the `azure_core`/`azure_identity`/`typespec_client_core` dependency stack went with the last one). Modules deleted: `jcode-base/src/auth/{copilot,cursor,azure}.rs` + their tests, `jcode-base/src/provider/{copilot,cursor}.rs`, `jcode-base/src/copilot_usage.rs`. Type surface: `ActiveProvider::{Copilot,Cursor}` (now Claude/OpenAI/Antigravity/Gemini/OpenRouter only) and the matching `ProviderAvailability` fields incl. `copilot_premium_zero`; `RuntimeKey::{Copilot,Cursor}`; `ModelRouteApiMethod::{Copilot,Cursor}` + `is_copilot()`/`is_cursor()`; the `PremiumMode` enum with `Provider::set_premium_mode`/`premium_mode` and `pricing::copilot_pricing`; `LoginProviderTarget::{Azure,Cursor,Copilot}`; `LoginProviderAuthStateKey::{Azure,Copilot,Cursor}`; the `AZURE_`/`CURSOR_`/`COPILOT_LOGIN_PROVIDER` consts (`LOGIN_PROVIDERS` 45 → 42); `AuthCredentialSource::AzureDefaultCredential`; six `AuthStatus` fields and their `probe_*` fns; `jcode-usage-types::{CopilotUsageTracker,DayUsage,MonthUsage,AllTimeUsage}`; `provider::activation::apply_azure_openai_runtime` + `RuntimeProviderId::{AzureOpenAi,Cursor,Copilot}`; `auth::external::load_copilot_oauth_token`. CLI: `ProviderChoice::{Azure,Copilot,Cursor}` (so `--provider copilot\|cursor\|azure` and the `azure-openai`/`aoai` aliases), `AuthTestTarget::{Copilot,Cursor}`, the interactive Azure setup flow, the Copilot GitHub device-code flow (CLI + TUI + scriptable), and the Cursor API-key flow. Config: `[provider] copilot_premium` + `Config::set_copilot_premium` + `JCODE_COPILOT_PREMIUM`. **Two second-order removals the purge exposed:** (a) `JCODE_OPENROUTER_DYNAMIC_BEARER_PROVIDER` and the whole `ProviderAuth::AzureEntra` dynamic-bearer path in `jcode-provider-openrouter-runtime` — `"azure"` was its only accepted value; (b) the `AuthStatus` parameter threaded into `MultiProvider`, because the Copilot/Cursor slots were its only readers — `new_fast()`/`from_auth_status()` collapsed into a single `new()` and startup no longer runs a throwaway `AuthStatus::check()`. Also gone: `jcode login --complete` (Copilot was the only provider whose completion path did not require explicit `--callback-url`/`--auth-code` input, so the flag could only error). **Kept:** Cursor *editor* transcript import — `~/.cursor/projects` discovery, `imported_cursor_*` session ids, `ResumeTarget::CursorSession`, `SessionSource::Cursor`, and the session-picker rows that list them. That is a transcript-import source, not a model provider. `normalize_copilot_model_name` was renamed `normalize_dotted_model_version` rather than deleted: it maps dotted versions (`claude-opus-4.6` → `claude-opus-4-6`) and is still used for Claude model names. |
...
- All 15 remaining `jcode-provider-*` crates and every OAuth flow
- Cursor **editor** transcript import (`~/.cursor/projects`, `imported_cursor_*`, `SessionSource::Cursor`) — survived the Cursor *provider* purge on purpose
...
copy/symlink dance and additionally calls `jcode server reload` and **edits your shell rc files**
via `jcode_configure_path` (`scripts/lib/configure_path.sh`) — do it manually to skip those. It no
longer installs the macOS launcher eagerly (the `setup-launcher` command was purged; the first
interactive launch installs `Jcode.app` plus the turn-notification broker via
`maybe_show_setup_hints`), and it no longer registers global launch hotkeys: that subsystem was
purged (§1).
...
`check_swallowed_error_budget.py`, `check_test_size_budget.py`) **already fail at upstream
HEAD**. Verify against a pristine `git worktree` before treating any as a regression.

They share `scripts/budget_common.py`, which gives all four the same ledger verbs. Bare
`--update` still rebases everything and still absorbs every pre-existing violation — prefer:

| verb | use |
|---|---|
| `--update PATH ...` | record only the paths you changed; refuses to record something *worse* without `--allow-regression` |
| `--moved OLD=NEW[,NEW...]` | re-key after a pure move or split; refuses the move unless the counts conserve |
| `--prune` | retire entries for files that no longer exist or no longer qualify |
| `--repair` | re-derive `total`/`totals_by_pattern` from `tracked_files` |

A ledger whose summary disagrees with its own table is now a hard error rather than a silent
source of phantom headroom: `panic_budget.json` shipped with `total: 72` against a table
summing to 58, so fourteen new panics would have passed the total check unnoticed.

Every guard self-checks before it reports. Each declares fixtures - lines (or
paths) it must count and must ignore - and refuses to draw a conclusion if its
own detector cannot satisfy them. Without that, a detector that stops matching
does not fail: it reports an improvement and exits 0. Neutering the panic
pattern turned `total=58` into `"shrank: 58 -> 0"` with a passing exit code.

The swallowed-error detector also excludes two shapes that provably discard no
error: `env::var(..).ok()` (an unset variable is a state, not a failure) and
`<option-returning call>.unwrap_or_default()`. `Result`-returning receivers such
as `map` are deliberately *not* excluded, and a fixture pins
`fs::read_to_string(p).map(..).unwrap_or_default()` as countable - the first
draft of that list wrongly excluded it.

### Dead surface

`scripts/check_dead_surface.py` (wired into `check_guardrails.sh`) fails when the repo grows
surface nothing consumes. Three detectors, each for a condition this fork has actually shipped:

| detector | what it caught before it existed |
|---|---|
| workspace member with no dependent manifest | `crates/jcode-sdk`, 4,398 lines, orphaned when the desktop app was deleted — it still compiled, so nothing objected |
| declared `[[bin]]` with no script/doc invoker | `jcode-harness-api-bridge`, whose only spawner lived inside that same orphaned crate |
| script referenced by no file in the repo | 47 scripts, 12,000+ lines |

**Why a guard and not an audit.** Deleting 43 orphaned scripts orphaned 4 more
(`repro_input_flicker.py`, `repro_real_spawn_lag.py`, `antigravity_live_coverage.sh`,
`run_terminal_bench_claude.sh`) — each had been referenced only by one of the deleted ones. The
audit that removed the 43 reported a fixpoint while those 4 were live orphans. Cascades are
invisible to a one-shot sweep and obvious to a re-run.

An orphan may be accepted, but only with a stated reason, in
`scripts/dead_surface_allowlist.json`:

```bash
scripts/check_dead_surface.py --allow <path> '<why nothing referencing it is fine>'
scripts/check_dead_surface.py --prune   # retire entries that are no longer orphans
```

Note: keep examples in this section generic (`<path>`). Naming a real path here counts as a
reference, which would quietly rescue that file from the detector - one of the allowlisted
harnesses was briefly reported as "no longer orphaned" for exactly that reason, because a
doc example named it.

An entry with an empty reason is a hard error. The 13 `test_*` harnesses are allowlisted because
reference counting is the wrong test for a script a human runs by hand — that judgement belongs
next to the path, not in someone's memory. Like the other guards, the detectors self-check
against planted fixtures first, so one that stops matching fails instead of reporting
"no dead surface".


Individual lines may opt out of the two counting budgets with a justified
`// budget-ok: <reason>` comment. An unexplained `// budget-ok` is not honoured — the point is
that the reason lives next to the code instead of inside an opaque number.
