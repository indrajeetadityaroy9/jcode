...
///
/// Creates the temp symlink, then renames it over the target (atomic).
...
/// Replace the current process with a new command (exec).
///
/// This calls exec(), which never returns on success.
///
...
pub fn replace_process(cmd: &mut std::process::Command) -> std::io::Error {
    use std::os::unix::process::CommandExt;
    let err = cmd.exec();
    crate::logging::error(&format!("replace_process failed: {err}"));
    err
}
