...
async fn websearch_is_exposed_by_default_and_can_be_explicitly_disabled() {
    ...
    // The ambient-cycle-only tools are the real "must not leak" set: they are
    // registered solely by `register_ambient_tools`, so a regular session must
    // never see them. This previously guarded a `debug_socket` tool that has
    // never existed in this fork, so it could not fail.
    for hidden in ["end_ambient_cycle", "schedule_ambient", "send_message"] {
        assert!(
            !tool_names.iter().any(|name| name == hidden),
            "ambient-only tool {hidden} must not be model-visible in regular sessions"
        );
    }
...
include!("agent_tests/part_02.rs");
