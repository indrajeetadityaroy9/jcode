                        ...
                        Err(e) => {
                            ...
                            if retry_after_secs.is_some() {
                                    ...
                                    || lower.contains("403 forbidden")
                                {
                                    ...
                                    // OAuth flow noise) and inflated the auth-failure
                                    // counter.
                                }
                            }
                        }
...
async fn cancel_processing_message(
    ...
    if let Some(mut handle) = state.task.take() {
        ...
        // Only clear the cancel we fired: a newer cancel (repeated Esc, another
        // connection) must not be erased before its target observes it
        // (issue #428).
