...
impl App {
    pub(super) fn handle_productivity_report_ready(&mut self, event: ProductivityReportReady) {
        ...
        match event.result {
            Ok(payload) => {
                let copied = copy_image_to_clipboard(&payload.png);
...
///
/// Goes through arboard, which expects raw RGBA, so we decode the PNG for it.
fn copy_image_to_clipboard(png: &[u8]) -> bool {
