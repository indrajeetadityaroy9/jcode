...
fn tier3_screen_score_w(m: &ScreenMetrics, w: &Tier3Weights) -> f64 {
    let mut score = 100.0;
    // Reading load: a lean screen sits around 8-12 lines. Penalize words past
    // a comfortable budget (title + one prompt + options + hint).
...
/// dropping the ASCII logo art and the Yes/No pill row. Returns the kept
/// prose lines.
