...
#[test]
fn test_cancel_clears_pending_rate_limit_retry() {
    let mut app = create_test_app();
    ...
    assert!(
        ...
        "/cancel must disarm the retry timer"
    );
    assert!(
        app.rate_limit_pending_message.is_none(),
        "/cancel must discard the pending retry payload"
    );
    assert!(
        !app.cancel_requested,
        "/cancel must not leak cancellation into the next turn"
    );
    assert_eq!(
        app.status_notice
            .as_ref()
