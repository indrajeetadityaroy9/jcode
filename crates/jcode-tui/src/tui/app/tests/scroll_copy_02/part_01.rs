...
fn test_local_tool_error_copy_badge_shortcut_supported() {
    ...
    let copied = clipboard
        .text()
        .expect("tool error should reach the clipboard");
    assert!(
...
fn test_local_blockquote_copy_badge_shortcut_supported() {
    ...
    let copied = clipboard
        .text()
        .expect("blockquote should reach the clipboard");
    assert!(
...
fn test_copy_selection_metrics_match_built_selection_text() {
    ...
    let (chars, lines) = crate::tui::ui::copy_selection_metrics(range).expect("selection metrics");
