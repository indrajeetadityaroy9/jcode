...
\n\
First inspect the codebase, current repo state, and the in-repo quality docs if they exist, especially `docs/REFACTORING.md` and `docs/plans/CODE_QUALITY_10_10_PLAN.md`, plus the live budget baselines in `scripts/*_budget.json` (oversized files, test size, panics, swallowed errors). Then write a concise ranked todo list using `todo` with the best 3-7 candidate refactors. Prefer behavior-preserving extraction, file splits, dead-code deletion, warning reduction, test isolation, and clearer module boundaries.\n\
\n\
...
\n\
First inspect the codebase, current repo state, and the in-repo quality docs if they exist, especially `docs/REFACTORING.md` and `docs/plans/CODE_QUALITY_10_10_PLAN.md`, plus the live budget baselines in `scripts/*_budget.json` (oversized files, test size, panics, swallowed errors). Then write a concise ranked todo list using `todo` with the best 3-7 refactors to tackle next. Prefer behavior-preserving extraction, splitting oversized modules, dead-code deletion, warning reduction, test improvements, and boundary clarification.{}\n\
\n\
